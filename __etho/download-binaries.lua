...
local function download_and_extract(url, dst, options)
    ...
    if not _ok then
        ...
        ami_error(_exString.join_strings("Failed to download: ", _error), EXIT_APP_DOWNLOAD_ERROR)
    end

    local _ok, _error = eliZip.safe_extract(_tmpFile, dst, options)
    eliFs.remove(_tmpFile)
    ami_assert(_ok, _exString.join_strings("Failed to extract: ", _error), EXIT_APP_DOWNLOAD_ERROR)
end
...
for _, file in ipairs(_files) do 
    ...
    local _ok, _error = eliFs.safe_chmod(file, "r-xr-xr-x")
    if not _ok then 
